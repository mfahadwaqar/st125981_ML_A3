...
# ==== Load MLflow Model (A3) ====
try:
    model_name = os.getenv("APP_MODEL_NAME", "st125981-a3-model")
    ...
    a3_model = None
    print(f"⚠️ Could not load A3 model from MLflow: {e}")
...
def input_form(model_name):
    return html.Div([
        html.H1(f"Car Price Prediction – {model_name.upper()}", style={"textAlign": "center"}),
        html.P("Fill in the details below to predict the car's selling price or class.", style={"textAlign": "center"}),
        html.Div([
            html.Div([
                html.Label("Year"), dcc.Input(id=f"year-{model_name}", type="number"),
                html.Label("Kilometers Driven"), dcc.Input(id=f"km_driven-{model_name}", type="number"),
...
navbar = html.Div([
    dcc.Link(html.Button("Home", style={"backgroundColor": "#333", "color": "white",
                                        "border": "none", "padding": "10px 20px", "borderRadius": "8px",
                                        "cursor": "pointer", "marginRight": "10px"}), href='/'),
    dcc.Link(html.Button("Old Model (A1)", style={"backgroundColor": "#007acc", "color": "white",
                                                  "border": "none", "padding": "10px 20px", "borderRadius": "8px",
                                                  "cursor": "pointer", "marginRight": "10px"}), href='/old'),
    dcc.Link(html.Button("Custom Model (A2)", style={"backgroundColor": "#ff5722", "color": "white",
                                                     "border": "none", "padding": "10px 20px", "borderRadius": "8px",
                                                     "cursor": "pointer", "marginRight": "10px"}), href='/new'),
    dcc.Link(html.Button("New Model (A3)", style={"backgroundColor": "#4CAF50", "color": "white",
                                                  "border": "none", "padding": "10px 20px", "borderRadius": "8px",
                                                  "cursor": "pointer"}), href='/a3'),
], style={"textAlign": "center", "padding": "15px", "backgroundColor": "#1e1e1e",
          "position": "sticky", "top": "0", "zIndex": "1000", "marginBottom": "30px"})
...
def display_page(pathname):
    if pathname == '/old':
        ...
        return html.Div([navbar, input_form("a3")])
    else:
        return html.Div([navbar, html.H1("Choose a model to start predicting.", style={"textAlign": "center"})])
...
@app.callback(
    ...
    Input("predict-btn-a3", "n_clicks"),
    State("year-a3", "value"), State("km_driven-a3", "value"),
    State("mileage-a3", "value"), State("engine-a3", "value"),
    State("max_power-a3", "value"), State("brand-a3", "value"),
    State("transmission-a3", "value"), State("owner-a3", "value"))
def predict_a3(n_clicks, year, km, mileage, engine, power, brand, trans, owner):
    if not n_clicks:
        return ""
    if a3_model is None:
        return "MLflow model not loaded."
    ...
    try:
        ...
        prediction = a3_model.predict(df_input)
        return f"Predicted Class: {int(prediction[0])}"
    except Exception as e:
        return f"Error during A3 prediction: {e}"
